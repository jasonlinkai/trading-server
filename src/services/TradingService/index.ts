 ...
 */
  protected findMatchingMarketBySymbol(symbol: string): ccxt.Market | false {
    ...
    console.log(`[${this.exchangeType}][MATCH] 查找 ${symbol} ${exchangeSymbol}的市場數據 - 比對API返回的市場數據，尋找最佳匹配`);
 ...
 */
  ...
  protected convertSymbolForExchange(symbol: string): string {
    console.log(`[${this.exchangeType}][SYMBOL] 開始轉換交易對: ${symbol} - 原因: 需要將通用符號轉換為 ${this.exchangeType} 專用格式`);
